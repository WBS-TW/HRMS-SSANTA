...
In this tutorial, I will guide you through the different steps to export the spectra information for each compound from a standard to contribute to the MTM HRMS spectral library. The project is organized in different subfolders within the main folder named "_MTM_HRMS_LIB"_. Do not change the name of any folders.   
...
In the new MS-FINDER window, fill in the information in (10) in Figure \@ref(fig:MSFINDER) for your compound. For GC-MS, in most cases the monoisotopic peak (equivalent to the precursor m/z) and you can choose the ion closest to the monoisotopic mass of your standard compound. Choose the precursor type. For GC-MS, choose [M]+. After you are done, choose "Fragment annotation (single)" in the ""Analysis" menu (11). Afterwards, choose the "Fragment annotation (batch job)" under the same menu. The _in silico_ annotation of the fragment ions should not be finished (12) and you can now export the annotated spectrum as an ".msp" file (13). Use the file name that was generated in the standard infosheet you filled in at the "COMMENT: msp file name" row. Save the file in the "3_MSP" subfolder.  
